...
This repository is an early-stage prototype. The working runtime capability is a PostgreSQL-backed Model Context Protocol (MCP) server for searching, retrieving, and creating IT incidents.

The repository also contains an initial database schema for users, entitlements, access requests, and audit logs. Those tables describe the intended direction, but the corresponding access-management, policy, agent, guardrail, approval, and audit workflows are not implemented yet.
...
The MCP server is created as `Incident Management Server` and exposes three tools:
...
| `app/services/access_service.py` | Intended access-request service | Empty placeholder. |
...
| `tests/` | Database and MCP smoke tests | Three basic tests; they require an already seeded database. |
...
Only `Incident` is used by the current service and MCP server. `AccessRequest` and `AuditLog` are schema foundations, not active workflows. Foreign keys are present, but ORM relationships, database check constraints, authentication, authorization, and tenant isolation are not defined.
...
An MCP-compatible client can then connect using the transport supported by the installed MCP package and call the three incident tools described above.
...
- No access-request evaluation or approval gate.
...
- No input validation at the service layer beyond MCP priority validation.
...
3. Implement access-request creation, risk evaluation, entitlement checks, and approval state transitions.
...
Today, this project is best understood as a foundation for an IT operations copilot: a local PostgreSQL schema plus an MCP incident server. Its immediate value is demonstrating the incident-tool boundary and reserving the data structures needed for future privileged-access automation. The secure multi-agent copilot described by the project name and original summary remains to be built.
